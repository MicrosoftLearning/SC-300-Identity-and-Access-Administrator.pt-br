...
2. Pesquise e selecione **Azure Active Directory**.

3. No painel de navegação à esquerda, em **Gerenciar**, selecione **Usuários**, **+ Novo Usuário** e **Criar novo usuário**.

4. Marque o botão **Criar usuário**. Crie um usuário com as seguintes informações:

    | **Configuração**| **Valor**|
    | :--- | :--- |
    | Nome de usuário| ChrisG|
    | Nome| Chris Green|
    | Nome| Chris|
    | Sobrenome| Verde|
...
6. Anote a senha gerada em um local de fácil acesso para a próxima tarefa.
     ...
     *Você terá que alterar a senha ao fazer login pela primeira vez nesta conta*

7. Selecione **Criar**. O usuário é criado e registrado em sua organização.

#### Tarefa 2 - Fazer login e tentar criar um aplicativo

1. Abra uma nova janela anônima do navegador.
2. Abra o Portal do Azure [https://portal.azure.com](https://portal.azure.com) como Chris Green.
...
3. Atualize a senha.
    ...
    | Confirmar senha| Digite novamente a senha única e segura |

4. Se você vir a caixa de diálogo do tour **Bem-vindo ao Microsoft Azure**, clique no botão **Talvez mais tarde**.

5. Pesquise e selecione **Aplicativos empresariais** na caixa de diálogo de pesquisa na parte superior da tela.
...
Usando o Azure Active Directory (Azure AD), você pode designar administradores limitados para gerenciar tarefas de identidade em funções com privilégios mínimos. Os administradores podem ser atribuídos para os fins de adicionar ou alterar os usuários, atribuir funções administrativas, redefinir senhas de usuário, gerenciar licenças de usuário e gerenciando nomes de domínio.

1. Se você ainda não tiver entrado como uma função de administrador global, abra o portal do Azure e faça logon.
2. Acesse a página do Azure Active Directory.
3. Selecione **Usuários** na seção Gerenciar do menu.
4. Selecione a conta de **Chris Green**.
...
**Observação** - Se o ambiente de laboratório já tiver ativado o Azure AD Premium P2, o Privileged Identity Management (PIM) estará habilitado e você precisará selecionar **Avançar** e atribuir uma função permanente a esse usuário.

8. Selecione o botão **Atualizar**.

**Observação - A função de Administrador de aplicativos recém-atribuída aparece na página Funções atribuídas do usuário.**
...
6. Clique em **+ Novo aplicativo**.

   **Observação - Essa função agora permite adicionar aplicativos ao locatário. Vamos experimentar um pouco mais esse recurso nos próximos laboratórios.**

7. Saia da instância de Chris Green no portal do Azure e feche o navegador.
...
1. Se você ainda não tiver entrado como administrador global, abra o portal do Azure e faça logon agora.
2. Na caixa de pesquisa, digite **Azure Active Directory** e inicie o Azure AD.
3. No **Azure Active Directory**, selecione  **Funções e administradores** e, em seguida, selecione a função **Administrador de aplicativos** na lista.

**Observação** - Se o ambiente de laboratório já tiver ativado o Azure AD Premium P2, o Privileged Identity Management (PIM) estará habilitado e você precisará selecionar **Avançar** e atribuir uma função permanente a esse usuário.

4. Na página **Administrador de aplicativos | Atribuições** você verá o nome de Chris Green listado.
...
7. Responda **Sim** quando a caixa de confirmação abrir.
8. Feche o Azure Active Directory.

### Exercício 4 - Importação em massa de usuários

#### Tarefa 1 - Operações em massa para criar usuários com um arquivo .csv

1. No menu do Azure AD, selecione **Usuários** em **Gerenciar**.
...
**Observação** - Selecione PowerShell e não PowerShell ISE.

2. Você precisará adicionar e importar o módulo do PowerShell do Azure AD se ainda não tiver usado ele.  Execute os dois comandos a seguir e confirme a operação quando solicitado pressionando Y:
...
3. Confirme se o módulo foi instalado corretamente executando o comando:  
...
5. A janela de logon da Microsoft aparecerá para você fazer logon no Azure AD.  

6. Para verificar se você está conectado e ver os usuários existentes, execute:  

    ``` 
    Get-AzureADUser 
    ```
    
7. Para atribuir uma senha temporária comum a todos os novos usuários, execute o seguinte comando e substitua o TempPW pela senha que você gostaria de fornecer aos usuários.  
...
8. Você está pronto para criar um novo usuário.  O comando a seguir será preenchido com as informações do usuário e executado.  Se você tiver mais de um usuário para adicionar, poderá usar um arquivo txt do bloco de notas para adicionar as informações e copiar/colar no PowerShell. 
...
Você pode adicionar e remover usuários na página do Azure AD.  No entanto, os usuários podem ser criados e as funções podem ser atribuídas usando o script.  Experimente dar à conta de usuário de Chris Green uma função diferente usando o script. 
 

### Exercício 5 - Remover um usuário do Azure Active Directory
...
7. Certifique-se de que Raul não tem "Nenhuma atribuição de licença encontrada".

8. Navegue até [https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview]( https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview).

9. No painel de navegação esquerdo, em **Gerenciar**, selecione **Usuários**.

10. Na página Usuários, selecione **Raul Razo**.

11. No painel de navegação esquerdo, selecione **Licenças**.

12. Selecione o botão **+ Atribuições**. 

13. Na página Atualizar atribuições de licença, marque a caixa de seleção de uma licença do **Windows 10/11 Enterprise E3**.
...
14. Ao concluir, selecione **Salvar**.

15. Na parte superior da tela, selecione **Página Inicial**, selecione **Contoso**, **Usuário** e clique em **Raul Razo**.

16. Observe que a licença foi atribuída.
